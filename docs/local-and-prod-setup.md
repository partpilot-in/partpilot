...
3. **Access Services**:
   ...
   * **API Server (`partpilot-server`)**: `http://localhost:8080`
   * **RSS Feed Server (`partpilot-feed`)**: `http://localhost:8090`
...
### 2. Run API Server (`partpilot-server`)

```bash
cargo run --bin partpilot-server
```
The server will start listening on `http://127.0.0.1:8080`.

### 3. Run RSS Feed Server (`partpilot-feed`)

```bash
cargo run --bin partpilot-feed
```
...
1. Copy `.env.example` to `.env` on your production host and configure:
   * Production `DATABASE_URL` / `POSTGRES_PASSWORD`
...
* **Server**: Deploy as a Rust Web Service using `Dockerfile.server`. Configure environment variables (`DATABASE_URL`, `REDIS_URL`, `SUPABASE_*`).
* **Worker**: Deploy `partpilot-worker` as a background worker service.
* **Feed**: Deploy `partpilot-feed` using `Dockerfile.feed`.
* **Client**: Deploy `client` as a static SPA site (or build container with `Dockerfile.client`).
